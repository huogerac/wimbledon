...
```bash
dk bash  # starts bash inside "wimbledon" container
...
- `dkup` uses docker-compose to start 3 containers: postgres, nginx, and wimbledon.
...
- Once `dkup` is running, `dk <command>` will run `<command>` inside the `wimbledon` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
- The wimbledon container runs 3 services:
- django on port 8000
...
```bash
mkvirtualenv wimbledon -p python3  # creates a python3 virtualenv
